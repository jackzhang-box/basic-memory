...
def get_bmignore_path() -> Path:
    ...
    Returns:
        Path to ~/.agent-brain/.bmignore
    """
    return Path.home() / ".agent-brain" / ".bmignore"


def create_default_bmignore() -> None:
    """Create default .bmignore file if it doesn't exist.

    This ensures users have a file they can customize for all Agent Brain operations.
    """
    ...
    bmignore_path.write_text("""# Agent Brain Ignore Patterns
...
# Agent Brain internal files (includes test databases)
*.db
*.db-shm
*.db-wal
config.json
...
def load_gitignore_patterns(base_path: Path, use_gitignore: bool = True) -> Set[str]:
    ...
    1. ~/.agent-brain/.bmignore (user's global ignore patterns)
