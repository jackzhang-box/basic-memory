...
from agent_brain.config import AgentBrainConfig, ConfigManager, DatabaseBackend
...
class DatabaseType(Enum):
    ...
    @classmethod
    def get_db_url(
        cls, db_path: Path, db_type: "DatabaseType", config: Optional[AgentBrainConfig] = None
    ) -> str:
...
def _create_postgres_engine(db_url: str, config: AgentBrainConfig) -> AsyncEngine:
    """Create Postgres async engine with appropriate configuration.

    Args:
        db_url: Postgres connection URL (postgresql+asyncpg://...)
        config: AgentBrainConfig with pool settings
...
async def run_migrations(
    app_config: AgentBrainConfig, database_type=DatabaseType.FILESYSTEM
):  # pragma: no cover
