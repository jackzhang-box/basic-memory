"""Git sync backend for Agent Brain.
...
class GitSyncBackend:
    ...
    async def init_repo(self) -> None:
        ...
        patterns = {"*.db", "*.db-journal", "*.db-wal", ".agent-brain/"}
        existing = set()
        if gitignore.exists():
    ...
    async def push(self, message: str | None = None) -> GitPushResult:
        ...
        # Commit
        commit_msg = message or f"sync: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')} via agent-brain"
