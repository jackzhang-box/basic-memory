"""Utility functions for agent-brain."""
...
def normalize_project_path(path: str) -> str:
    ...
    Args:
        path: Project path (e.g., "/app/data/agent-brain-llc" or "C:\\Users\\...")

    Returns:
        Normalized path (e.g., "/agent-brain-llc" or "C:\\Users\\...")
...
def setup_logging(
    ...
    Args:
        ...
        log_to_file: Write to ~/.agent-brain/agent-brain.log with rotation
    ...
    env = os.getenv("AGENT_BRAIN_ENV", "dev")
    if env == "test":
    ...
    if log_to_file:
        ...
        # Why: multiple agent-brain processes can share the same log directory at once.
        # Outcome: use per-process log files on Windows so log rotation stays local.
        log_filename = f"agent-brain-{os.getpid()}.log" if os.name == "nt" else "agent-brain.log"
        log_path = Path.home() / ".agent-brain" / log_filename
...
def _cleanup_windows_log_files(log_dir: Path, current_log_name: str) -> None:
    ...
    stale_logs = [
        path
        for path in log_dir.glob("agent-brain-*.log*")
    ...
    # Why: loguru retention applies per sink, not across the whole agent-brain log directory.
