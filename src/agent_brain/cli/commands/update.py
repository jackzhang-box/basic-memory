"""Manual update command for Agent Brain CLI."""
...
def update(
    ...
    if result.status == AutoUpdateStatus.UPDATED:
        console.print(f"[green]{result.message or 'Agent Brain updated successfully.'}[/green]")
        return

    if result.status == AutoUpdateStatus.UP_TO_DATE:
        console.print(f"[green]{result.message or 'Agent Brain is up to date.'}[/green]")
        return
