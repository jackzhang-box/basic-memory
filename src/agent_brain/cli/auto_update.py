"""Automatic update checks and upgrades for the Agent Brain CLI."""
...
PACKAGE_NAME = "agent-brain"
PYPI_JSON_URL = "https://pypi.org/pypi/agent-brain/json"
...
def detect_install_source(executable: str | None = None) -> InstallSource:
    ...
    if "cellar/agent-brain" in normalized:
        return InstallSource.HOMEBREW
    if "uv/tools/agent-brain" in normalized:
...
def _version_from_pypi() -> str:
    ...
    request = urllib.request.Request(
        PYPI_JSON_URL,
        headers={"User-Agent": f"agent-brain-cli/{agent_brain.__version__}"},
    )
...
def _check_homebrew_update_available(silent: bool) -> tuple[bool, str | None]:
    """Check whether Homebrew reports an outdated agent-brain formula."""
...
def _check_pypi_update_available() -> tuple[bool, str]:
    ...
    try:
        current_version = Version(agent_brain.__version__)
        latest_version = Version(latest)
    except InvalidVersion as exc:
        raise RuntimeError(
            f"Could not compare versions (current={agent_brain.__version__}, latest={latest})"
        ) from exc
...
def _manual_update_hint(source: InstallSource) -> str:
    ...
    if source == InstallSource.UV_TOOL:
        return "Run `uv tool upgrade agent-brain`."
    if source == InstallSource.HOMEBREW:
        return "Run `brew upgrade agent-brain`."
    return (
        "Automatic install is not supported for this environment. "
        "Update with your package manager (for pip: `python3 -m pip install -U agent-brain`)."
    )
    ...
    try:
        ...
        if not update_available:
            return AutoUpdateResult(
                ...
                message=f"Agent Brain is up to date ({agent_brain.__version__}).",
            )
        ...
        return AutoUpdateResult(
            ...
            latest_version=latest_version,
            message=(
                "Agent Brain was updated successfully. "
