"""Schema tools for Agent Brain MCP server.
...
from agent_brain.schemas.schema import DriftReport, InferenceReport, ValidationReport
...
def _no_schema_guidance(note_type: str, tool_name: str) -> str:
    ...
    return (
        ...
        f"3. **Sync** — run `agent-brain sync` or wait for auto-sync to pick up "
    ...
    async with get_project_client(project, context=context) as (client, active_project):
        ...
        try:
            ...
            return (
                ...
                f"3. Verify the project has been synced: `agent-brain status`\n"
            )
