"""Search tools for Agent Brain MCP server."""
...
from agent_brain.utils import coerce_dict, coerce_list
from agent_brain.mcp.container import get_container
from agent_brain.mcp.project_context import (
...
def _format_search_error_response(
    ...
    if "semantic search is disabled" in error_message.lower():
        return dedent(f"""
            ...
            ## How to enable
            1. Set `AGENT_BRAIN_SEMANTIC_SEARCH_ENABLED=true`
            2. Restart the Agent Brain server/process
    ...
    if "pip install" in error_message.lower() and "semantic" in error_message.lower():
        return dedent(f"""
            ...
            ## Fix
            1. Install/update Agent Brain: `pip install -U agent-brain`
            2. Restart Agent Brain
            3. Retry your query:
...
@mcp.tool(
    ...
    # meta={"ui/resourceUri": "ui://agent-brain/search-results"},
        ...
        async with get_project_client(project, context=context) as (client, active_project):
            with telemetry.contextualize(
                ...
                try:
                    ...
                    from agent_brain.mcp.clients import SearchClient
