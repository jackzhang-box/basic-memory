...
from agent_brain.db import (
    scoped_session,
    _needs_semantic_embedding_backfill,
    _run_semantic_embedding_backfill,
)
from agent_brain.mcp.container import McpContainer, set_container
...
async def lifespan(app: FastMCP):
    ...
    with telemetry.operation(
        ...
        logger.info(f"Starting Agent Brain MCP server (mode={container.mode.name})")
        logger.info(
    ...
    try:
        ...
        with telemetry.operation(
            ...
            logger.debug("Shutting down Agent Brain MCP server")
