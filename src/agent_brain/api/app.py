"""FastAPI application for agent-brain knowledge graph API."""
...
from agent_brain import __version__ as version
from agent_brain.api.container import ApiContainer, set_container
from agent_brain.api.v2.routers import (
...
from agent_brain.api.v2.routers.project_router import (
    add_project,
    list_projects,
    synchronize_projects,
)
from agent_brain import telemetry
from agent_brain.config import init_api_logging
from agent_brain.services.exceptions import EntityAlreadyExistsError
from agent_brain.services.initialization import initialize_app
...
async def lifespan(app: FastAPI):  # pragma: no cover
    ...
    with telemetry.operation(
        ...
        logger.info(f"Starting Agent Brain API (mode={container.mode.name})")
    ...
    with telemetry.operation(
        ...
        logger.info("Shutting down Agent Brain API")
...
app = FastAPI(
    ...
    description="Knowledge graph API for agent-brain",
    version=version,
    lifespan=lifespan,
)
