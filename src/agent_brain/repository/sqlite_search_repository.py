...
from agent_brain.repository.embedding_provider_factory import create_embedding_provider
from agent_brain.repository.search_index_row import SearchIndexRow
from agent_brain.repository.search_repository_base import SearchRepositoryBase
from agent_brain.repository.metadata_filters import parse_metadata_filters, build_sqlite_json_path
...
class SQLiteSearchRepository(SearchRepositoryBase):
    ...
    def __init__(
        ...
        session_maker,
        project_id: int,
        app_config: AgentBrainConfig | None = None,
    ...
    async def _ensure_sqlite_vec_loaded(self, session) -> None:
        ...
        try:
            ...
            raise SemanticDependenciesMissingError(
                ...
                "Install/update agent-brain to include semantic dependencies: "
                "pip install -U agent-brain"
            ) from exc
    ...
    async def search(
        ...
        # --- Scoring strategy ---
        # Trigger: bm25() only produces meaningful scores when MATCH is in scope
        # Why: without MATCH, bm25 returns -0.0 for every row
        # Outcome: use bm25 only when text search is active; fall back to recency otherwise
        has_fts_match = bool(match_conditions)

        # Trigger: MATCH + entity JOIN fails with "unable to use function MATCH"
        # Why: FTS5 MATCH/bm25 require the FTS table as the direct FROM target
        # Outcome: when metadata filters need entity JOIN, use a two-pass approach:
        #   1. Query FTS table directly for rowids + bm25 scores
        #   2. Use those rowids to filter the outer query with entity JOIN
        use_two_pass = bool(metadata_filters and match_conditions)

        if use_two_pass:
            # match_conditions stay out of the main query — handled in pre-query
            pass
        elif has_fts_match:
            conditions.extend(match_conditions)
        # (else: no text search, no match conditions to add)

        if has_fts_match:
            primary_order = f"score ASC{order_by_clause}"
        else:
            primary_order = "search_index.updated_at DESC"
        ...
        if use_two_pass:
            # --- Two-pass: FTS query first, then metadata filter with scores ---
            match_where = " AND ".join(match_conditions)
            fts_sql = f"""
                SELECT search_index.rowid as fts_rowid,
                       bm25(search_index, 10.0, 5.0, 1.0, 1.0) as fts_score
                FROM search_index
                WHERE {match_where}
                AND search_index.project_id = :project_id
            """
            score_expr = "fts_scores.fts_score"
        else:
            fts_sql = ""
            score_expr = "bm25(search_index, 10.0, 5.0, 1.0, 1.0)" if has_fts_match else "0.0"
        ...
        try:
            async with db.scoped_session(self.session_maker) as session:
                if use_two_pass:
                    # Pass 1: get matching rowids + bm25 scores from FTS table directly
                    fts_result = await session.execute(text(fts_sql), params)
                    fts_rows = {row.fts_rowid: row.fts_score for row in fts_result.fetchall()}
                    if not fts_rows:
                        rows = []
                    else:
                        # Build temp table with scores for JOIN
                        await session.execute(
                            text(
                                "CREATE TEMP TABLE IF NOT EXISTS _fts_scores "
                                "(rowid INTEGER PRIMARY KEY, fts_score REAL)"
                            )
                        )
                        await session.execute(text("DELETE FROM _fts_scores"))
                        for rowid, fts_score in fts_rows.items():
                            await session.execute(
                                text(
                                    "INSERT INTO _fts_scores (rowid, fts_score) "
                                    "VALUES (:rowid, :fts_score)"
                                ),
                                {"rowid": rowid, "fts_score": fts_score},
                            )
                        # Pass 2: join with entity for metadata filtering
                        two_pass_sql = sql.replace(
                            "FROM search_index JOIN entity ON search_index.entity_id = entity.id",
                            "FROM search_index"
                            " JOIN entity ON search_index.entity_id = entity.id"
                            " JOIN _fts_scores fts_scores ON search_index.rowid = fts_scores.rowid",
                        )
                        result = await session.execute(text(two_pass_sql), params)
