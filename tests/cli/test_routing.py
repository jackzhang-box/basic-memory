...
class TestForceRouting:
    ...
    def test_local_sets_env_var(self):
        """Local flag should set AGENT_BRAIN_FORCE_LOCAL."""
        os.environ.pop("AGENT_BRAIN_FORCE_LOCAL", None)

        with force_routing(local=True):
            assert os.environ.get("AGENT_BRAIN_FORCE_LOCAL") == "true"

        # Should be cleaned up after context exits
        assert os.environ.get("AGENT_BRAIN_FORCE_LOCAL") is None
    ...
    def test_preserves_original_env_var(self):
        ...
        os.environ["AGENT_BRAIN_FORCE_LOCAL"] = "original"

        with force_routing(local=True):
            assert os.environ.get("AGENT_BRAIN_FORCE_LOCAL") == "true"

        # Should restore original value
        assert os.environ.get("AGENT_BRAIN_FORCE_LOCAL") == "original"
